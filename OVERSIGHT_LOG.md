...
| 2026-09-25 | b7edf6e | design | acceptance | self | A StructBlock rendered through include_block had no template, so every blog post with an in-body image displayed the block's own field names and the image's filename as body text. | Added a figure template rendering the image with an optional caption, and swept every block class to establish which are rendered by include_block. | no |
| 2026-09-26 | 36785ea | data-model | acceptance | self | The importer promoted the first body image to featured_image and left it in the body as well, so every imported post with an image rendered it twice once the block had a template. | Skipped the promoted image when building the body, and extended the repair path to fix already-imported posts while protecting any a human has since republished. | yes |
| 2026-09-26 | e70a9db | design | in-loop | agent-critique | A StructBlock with no template was offered inside a StreamField rendered with include_block, so the first FAQ added to a reading would have rendered as a definition list of field names. | Removed the block from that body, leaving it only where it renders correctly field-by-field, after confirming no existing content used it. | no |
